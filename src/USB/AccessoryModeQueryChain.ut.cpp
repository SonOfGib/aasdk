...
TEST_F(AccessoryModeQueryChainUnitTest, AccessoryModeQueryChain_QueryAOAPDevice)
{
    AccessoryModeQueryChain::Pointer queryChain(std::make_shared<AccessoryModeQueryChain>(usbWrapperMock_, ioService_, queryFactoryMock_));
        ...
        EXPECT_CALL(queryFactoryMock_, createQuery(AccessoryModeQueryType::PROTOCOL_VERSION, _)).WillOnce(
            DoAll(SaveArg<1>(&usbEndpoint), Return(queryMock_)));
        queryChain->start(deviceHandle_, std::move(promise_));
        ioService_.run();
        ioService_.reset();

        queryPromise->resolve(usbEndpoint);
        EXPECT_CALL(queryFactoryMock_, createQuery(AccessoryModeQueryType::SEND_MANUFACTURER, usbEndpoint)).WillOnce(
            Return(queryMock_));
        ioService_.run();
        ioService_.reset();

        queryPromise->resolve(usbEndpoint);
        EXPECT_CALL(queryFactoryMock_, createQuery(AccessoryModeQueryType::SEND_MODEL, usbEndpoint)).WillOnce(
            Return(queryMock_));
        ioService_.run();
        ioService_.reset();

        queryPromise->resolve(usbEndpoint);
        EXPECT_CALL(queryFactoryMock_, createQuery(AccessoryModeQueryType::SEND_DESCRIPTION, usbEndpoint)).WillOnce(
            Return(queryMock_));
        ioService_.run();
        ioService_.reset();

        queryPromise->resolve(usbEndpoint);
        EXPECT_CALL(queryFactoryMock_, createQuery(AccessoryModeQueryType::SEND_VERSION, usbEndpoint)).WillOnce(
            Return(queryMock_));
        ioService_.run();
        ioService_.reset();

        queryPromise->resolve(usbEndpoint);
        EXPECT_CALL(queryFactoryMock_, createQuery(AccessoryModeQueryType::SEND_URI, usbEndpoint)).WillOnce(
            Return(queryMock_));
        ioService_.run();
        ioService_.reset();

        queryPromise->resolve(usbEndpoint);
        EXPECT_CALL(queryFactoryMock_, createQuery(AccessoryModeQueryType::SEND_SERIAL, usbEndpoint)).WillOnce(
            Return(queryMock_));
        ioService_.run();
        ioService_.reset();

        queryPromise->resolve(usbEndpoint);
        EXPECT_CALL(queryFactoryMock_, createQuery(AccessoryModeQueryType::START, usbEndpoint)).WillOnce(
            Return(queryMock_));
        ioService_.run();
        ioService_.reset();
...
TEST_F(AccessoryModeQueryChainUnitTest, AccessoryModeQueryChain_RejectWhenInProgress)
{
    ...
    queryChain->start(deviceHandle_, std::move(promise_));
        ...
        secondPromise->then(std::bind(&AccessoryModeQueryChainPromiseHandlerMock::onResolve, &promiseHandlerMock_,
                                      std::placeholders::_1),
